...
def selector(dataset="mimic", degree=order[1], train=True):
    ...
    dset = h5py.File(write_path + "case(" + str(order[-1]) + ")_len(" + str(data_len) + ")_"
                     + str(int(param["chunk_size"] / sampling_rate["base"]) * samp_rate)
                     + "_train(" + str(train)+").hdf5", "w")
