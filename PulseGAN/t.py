...
def train_result(file_name):
	b=open('./results/generate_{}_80.csv'.format(file_name),'r')
	rdr2 = csv.reader(b)
	result2 = []
...
def clean_result(file_name):
	rdr3 = csv.reader(open('./data/compare/clean/{}.csv'.format(file_name),'r'))
	for data in rdr3:
	  return list(map(float, data))

def noisy_result(file_name): 
	rdr4 = csv.reader(open('./data/compare/noisy/{}.csv'.format(file_name),'r'))
	for data in rdr4:	  
	  return list(map(float, data))
...
for f in file_list:
    ...
    plt.savefig('./data/compare/result_img/{}.png'.format(f))
    plt.clf()
