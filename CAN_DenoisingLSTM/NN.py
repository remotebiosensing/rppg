...
class DeepPhys(torch.nn.Module):
    ...
    def forward(self, A, M):
        ...
        M2 = torch.tanh(self.m_Batch_Normalization2(self.m_conv2(M1)))
        ...
        M6 = torch.tanh(self.m_Batch_Normalization4(self.m_conv4(M5)))
        ...
        g2 = torch.mul(mask2, M7)

        M8 = self.m_avg1(g2)
        # M8 = self.m_Dropout1(M7)

        # Fully connected
        M9 = torch.flatten(M8, start_dim=1)
        M10 = torch.tanh(self.fully1(M9))
        M11 = self.fully_Dropout(M10)
        out = self.fully2(M11)
...
class LSTM(torch.nn.Module):
    def __init__(self):
        ...
        self.lstm = torch.nn.LSTM(input_size=n_examples, hidden_size=128, num_layers=n_numbers, batch_first=True,
                                  bidirectional=False)
        print("test")

    def forward(self, noise):
        lstm1 = self.lstm(noise)
        return lstm1
