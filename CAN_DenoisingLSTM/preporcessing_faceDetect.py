...
#     for folder in range(0, 4):
        ...
        while cap.isOpened():
            ...
            dst = cv2.resize(frame[out_result[0][1]-b:out_result[0][3]+int(b/2), out_result[0][12]-a:out_result[0][14]+a], dsize=(36, 36), interpolation=cv2.INTER_CUBIC)
            # cv2.imshow('dst', dst)
            ...
            # cv2.imshow('Motion',M.permute(1,2,0).numpy())
...
np.savez_compressed("./preprocessing/dataset/" + "UBFC_testset_face_49",
