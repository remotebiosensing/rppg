...
for sub_cnt in range(1,38):
    for folder in range(0,1):
        ...
        Name_save = np.concatenate((Name_save, Name_list), axis=0)
        ...
        Name_save = np.delete(Name_save, 0)
        print('test')
...
np.savez_compressed("./preprocessing/dataset/" + "COHFACE_normalize_non_test_",
                    A=Appearance_save, M=Motion_save, T=Target_save, N=Name_save)
