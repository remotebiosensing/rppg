...
from utils.funcs import detrend as dt
...
class VHRMethod(metaclass=ABCMeta):
    ...
    def runOffline(self, **kwargs):
        ...
        while T <= endTime:
            ...
            # -- save the estimate
            if  count == 0  or  bvpChunk.data[0].shape[0] != length*3:
                length = bvpChunk.data[0].shape[0]
                bvpES = np.empty((0, length*3), int)
                pad = length*3 - bvpChunk.data[0].shape[0]
                tmp = np.pad(bvpChunk.data[0],(0,pad),'constant', constant_values=0)
                bvpES = np.append(bvpES, [tmp], axis=0)
                count = count+1
            else:
                bvpES = np.append(bvpES, bvpChunk.data,axis=0)
           # bvpES.append(bvpChunk.data)
