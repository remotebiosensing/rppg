...
import numpy as np
# import cnibp.preprocessing.utils.multi_processing as multi
# import cnibp.preprocessing.utils.data_shuffler as ds
import time
import h5py
import datetime as dt
# import cnibp.preprocessing.utils.sutemp as sutemp
...
'''
1. mimic-iii waveform dataset에서 사람 id를 얻어온다.
2. mimic-iii clinical dataset에서 id에 맞는 의료 정보를 들고 온다.
3. 의료정보(성별 / 질병) 기반으로 사람을 나눠 놓는다.
4. 아주 기본적인 preprocessing(slicing, nan detection)만 한 뒤 overdamped, underdamped, device error(lrflip) 해결
5. dbp, sbp detection 하고 diff 구해서 튄 값 잡기
6. mimic3temp, mimic3matched, normalize preprocessing refactoring
7. visualization 함수들 통일 및 dataset 분석 용이랑 모델 결과 plot 찍는것 분리
8. 주의 : 데이터 클렌징이랑 모델에 넣을 것이랑 제대로 구분.
9. 자꾸 재활용되는 함수들 제거 및 통합
'''
...
def get_total_segment_path(read_path: str, gender: int):
    ...
    * if single patient have too many records,
    ...
    for tr in tqdm(train_path, desc='Train_segment'):
        ...
        #     continue
        if len(train_segments) > 50:
            reduced_train_segments = random.sample(train_segments, 50)
    ...
    for v in tqdm(val_path, desc='Val_segment'):
        ...
        #     continue
        if len(val_segments) > 50:
            reduced_val_segments = random.sample(val_segments, 50)
    ...
    for te in tqdm(test_path, desc='Test_segment'):
        ...
        #     continue
        if len(test_segments) > 50:
            reduced_test_segments = random.sample(test_segments, 50)
    ...
    return list_shuffler(train_shuffled_path), list_shuffler(val_shuffled_path), list_shuffler(
        test_shuffled_path), patient_info, patient_df
...
def read_total_data(id: int, segment_list: list, total_patient_info, patient_info_total: list,
    ...
    ** if a single record is too long,
    ...
    for segment in tqdm(segment_list, desc='process-' + str(id), leave=False):
        ...
        if len(ple) < 1000 or len(abp) < 1000:
            continue
        else:
            nan_mask = ~(np.isnan(ple) | np.isnan(abp))
            ...
            ple = (((ple_digital_sig - ple_baseline) / ple_gain) * (1023. / ple_gain))[nan_mask]
            ''' ************************************************************************* '''
            abp = abp[nan_mask]
            flat_mask = np.where(abp > 0.)
            abp = abp[flat_mask]
            ple = ple[flat_mask]
        # abp_record = wfdb.rdrecord(segment, channels=[abp_idx])
        # abp_sig_len = abp_record.sig_len
        # abp_analogue_sig = np.squeeze(abp_record.p_signal[:750])
        # abp_gain = abp_record.adc_gain
        # abp_init_value = abp_record.init_value
        # abp_adc_zero = abp_record.adc_zero
        # abp_digital_sig = np.squeeze(abp_record.adc()[:750])
        ...
        for raw_abp, raw_ple, target_abp, target_ple, target_peak_abp, target_peak_ple, target_bottom_abp, target_bottom_ple \
            ...
            if match_flag and len(matched_abp_peaks) > 0 and len(matched_abp_bottoms) > 0 \
                ...
                if signal_QC and np.max(matched_raw_abp[matched_abp_bottoms]) < 130 and \
                        np.min(matched_raw_abp[matched_abp_bottoms]) > 30 and \
                        np.max(matched_raw_abp[matched_abp_peaks]) < 240 and \
                        np.min(matched_raw_abp[matched_abp_peaks]) > 50:
                    '''
                    여기 문제@#@@@@@@@@@@@@@ sBP DBP 최종에서 다시 구해야함 
                    '''
                    # sdtw(np.reshape(raw_ple[:750][15:735:2], (-1, 1)), np.reshape(raw_abp[:750][15:735:2], (-1, 1)), gamma=0.)
                    # total : 0 / male : 1 / female : 2
                    # gender, expire_flag = total_patient_info[patient_id]
                    '''******************여기에 ple랑 abp softdtw 
                    ...
                    *************************'''
                    try:
                        ...
                        abp_total.append(raw_abp[:750][15:735:2])
                        # dist_array = cdist(np.reshape(ple_total[-1][0], (-1, 1)), np.reshape(abp_total[-1], (-1, 1)))
                        # x, sim = metrics.soft_dtw_alignment(ple_total[-1][0], abp_total[-1], 0.)
                        # x, sim = sdtw(ple_total[-1], abp_total[-1], gamma=0.01)
...
def multi_processing(model_name, dset_path: str, dataset: str, total_segments, g_str, total_patient_info, threshold,
                     ple_scale):
    '''
    param:
        ...
        with mp.Manager() as manager:
            ...
            workers = [mp.Process(target=read_total_data,
                                  args=(i, segments_per_process[i], total_patient_info,
                                        ...
                                        sig_len, samp_rate, eliminated_total, threshold, ple_scale)) for i in
                       range(process_num)]
...
def dataset_split(model_name: str, data_path: str, g_str: str, threshold: float, ple_scale: bool):
    ...
    if ple_scale:
        ...
        ssd_path = '/home/paperc/PycharmProjects/dataset/BPNet_mimiciii/additional_' + date + '/'
    print('dataset splitting...')
    train_segments, val_segments, test_segments, total_patient_info, patient_df = get_total_segment_path(data_path,
                                                                                                         gender=g_info[
                                                                                                             0])
    ...
    multi_processing(model_name, dset_path, 'train', train_segments, g_info[-1], total_patient_info, threshold,
                     ple_scale)
...
for g in gen:
    dataset_split('BPNet', '/hdd/hdd1/dataset/mimiciiisubset/physionet.org/files/mimic3wdb-matched/1.0', g_str=g,
                  threshold=0.9, ple_scale=False)
