...
from utils.train_utils import get_model, model_save
...
with open('configs/parameter.json') as f:
...
def main(model_name, dataset_name, in_channel, epochs, batch_size, wandb_on, gen):
    # sound.play()
    ...
    # model_vis.dbp_sbp_distribution_checker(dataset)
    train_cost_arr = []
    val_cost_arr = []
    test_cost_arr = []
    save_point = []
...
if __name__ == '__main__':
    ...
    for b in batch:
        for c in ch:
            main(model_name='BPNet', dataset_name='mimiciii', in_channel=c, epochs=100, batch_size=b,
                 wandb_on=True, gen="Total")
