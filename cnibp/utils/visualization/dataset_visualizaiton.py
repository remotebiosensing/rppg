...
channel = 3
batch_size = 512
dataset = dataset_loader(dataset_name=dataset_name, in_channel=[0, 1, 2], batch_size=1, device='cpu', gender='Total')
# dataset = dataset_loader(dataset_name=dataset_name, in_channel=channel_info[-1], batch_size=batch_size,
#                          device=DEVICE, gender=gender_info[-1])
def emd(dataset):
...
def get3dplot(dataset):
    '''
    plot 3d projection with PPG and ABP signal
    this function shows that amplitude of PPG signal does not define blood pressure.
    '''
    train_dataset = dataset[0]
    ...
    for ple, abp, d, s, _, info, o in train_dataset:
        cnt += 1
        # color = np.array([(cmax - cmin) * np.random.random_sample() + cmin for i in range(360)])
        ple_sig = ple[0][0].view(360, ).detach().cpu().numpy()
        abp_sig = abp[0].view(360, ).detach().cpu().numpy()
        if np.mean(ple_sig) < 1.:
            '''
            현재 데이터셋은 gain 기준으로 다 키워놔서 low_cnt 안나옴.
            '''
            low_cnt += 1
        ...
        if low_cnt + high_cnt > 2 and low_cnt > 1:
            break
...
def ple_clustering(dataset, test_n=100, cluster_num=5, check_point_n=10, plot_shape_flag=False,
    ...
    for ple, abp, d, s, _, info, o in tqdm(test_dataset):
        ple_sig = (ple[0][0].view(360, 1)).detach().cpu().numpy()
        abp_sig = (abp[0].view(360, 1)).detach().cpu().numpy()
        hf = 8
