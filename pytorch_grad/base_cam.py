...
class BaseCAM:
    ...
    def forward(self,
        ...
        cam_per_layer = self.compute_cam_per_layer(input_tensor,
                                                   targets,
                                                   eigen_smooth)
        return cam_per_layer#self.aggregate_multi_layers(cam_per_layer)
    ...
    def compute_cam_per_layer(
        ...
        for i in range(len(self.target_layers)):
            ...
            cam = self.get_cam_image(input_tensor,
                                     ...
                                     eigen_smooth)
            if self.ver == 0:
                cam = np.reshape(cam, (200, 32, 4, 4))
            elif self.ver == 1:
                cam = np.reshape(cam, (200, 64, 4, 4))
                cam = torch.tensor(cam)
                cam = F.interpolate(cam, scale_factor=(8, 1)).numpy()
