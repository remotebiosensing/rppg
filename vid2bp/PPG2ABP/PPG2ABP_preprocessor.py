...
def preprocessing(original_data_path, save_path, length, mode):
    ...
    # multiprocessing
    proc = multiprocessing.Process(target=preprocess_approximate_Dataset,
                                   args=(data_file['abp'], data_file['ple'][:,0], length, return_dict))
    proc.start()
...
def preprocess_approximate_Dataset(ABP, PLE, length, return_dict):
    ABP = ABP[:, :length]
    PLE = PLE[:, :length]
