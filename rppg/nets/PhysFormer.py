...
class PhysFormer(nn.Module):
    ...
    def __init__(
            ...
            patches: int = (4, 4, 4),  # The targeted tube size Ts × Hs × Ws equals to 4×4×4
            dim: int = 96,  # channel dimension D = 96
            ff_dim: int = 144,  # hidden dimension in ST-FF D = 144 are used for PhysFormer
            num_heads: int = 4,  # transformer heads h = 4
            num_layers: int = 12,  # The numbers of temporal difference transformer blocks N = 12
            ...
            theta: float = 0.7,  # PhysFormer could achieve smaller RMSE when theta is 0.4 and 0.7
            image_size: int = (64, 128, 128),
    ):
        super().__init__()
        if frame:
            image_size = (frame,) + image_size[1:]
...
class MultiHeadedSelfAttention_TDC_gra_sharp(nn.Module):
    ...
    def forward(self, x, gra_sharp=2.0):  # [B, 4*4*40, 128]
        """
