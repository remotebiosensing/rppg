...
class TSM(nn.Module):
    def __init__(self, n_clip=4, fold_div=3):
        super().__init__()
        self.n_clip = n_clip
    ...
    def forward(self, input):
        B, C, H, W = input.shape
        input = input.view(self.n_clip, -1, C, H, W)
        ...
        up_out1 = torch.cat((torch.zeros((self.n_clip, 1, fold, H, W)).to(device), out1[:, 1:, :, :, :]), dim=1)
        down_out2 = torch.cat((out2[:, :-1, :, :, :], torch.zeros((self.n_clip, 1, fold, H, W)).to(device)), dim=1)
